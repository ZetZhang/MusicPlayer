...
/**
 * @brief  init_UI UI初始化（.ui文件中无法完成的设置）
 */
...
/**
 * @brief  init_subpage 添加网络搜索功能，将组件添加到stackedWidget
 */
void MainWidget::init_subpage()
{
...
/**
 * @brief  init_play 初始化生成播放器模块及相关组件
 */
void MainWidget::init_play()
{
    ...
    // 禁止追踪：slider 在拖拽过程中不触发 valueChanged 信号，只有当用户释放 slider 的时候，才会触发 valueChanged 信号
    //    ui->positionSlider->setTracking(false);
    // 连接槽与信号
    connect(ui->positionSlider, &QAbstractSlider::valueChanged, this, &MainWidget::setPosition);
    // 滑动时播放的暂停与恢复
    connect(ui->positionSlider, &QAbstractSlider::sliderPressed, [this] { player->pause(); });
    connect(ui->positionSlider, &QAbstractSlider::sliderReleased, [this] { player->play(); });
...
void MainWidget::init_actions()
{
    ...
    QAction *action_namelist_delete = new QAction(QIcon(":/image/image/image/remove_list.png"), u8"删除");
...
void MainWidget::init_sqlite()
{
    ...
    if (query.next()) {
        if (query.value(0).toInt() == 0) {
            QSqlQuery sql_query;
            QString create_sql = "create table MusicInfo ("
                                    "name varchar(30),"
                                    "url varchar(200),"
                                    "author varchar(50),"
                                    "title varchar(50),"
...
/**
 * @brief  init_settings 配置初始化
 * @deprecated 将配置操作移至SettingManager更好
 */
...
//    if (mysettings.value("songDownload/location").toString().isEmpty()) {
//        QMessageBox::about(this, "初始化", "第一次打开本软件，请选择歌曲下载目录");
...
/**
 * @brief  updateDuration 无音乐播放时，界面元素
 *
 * @param  duration
 */
void MainWidget::updateDuration(qint64 duration)
{
    ...
    if(!(static_cast<int>(duration) > 0)) {
        ...
        mySystemTray->setToolTip(u8"MusicPlayer");
...
/**
 * @brief  setPosition avoid seeking when the slider value change is triggered from updatePosition()
 *
 * @param  position
 */
...
void MainWidget::updateInfo()
{
    if (player->isMetaDataAvailable()) {
        ...
        // QMessageBox::about(Q_NULLPTR, "0000", listInfo_debug.join(","));
        //歌曲信息
        QString info="";
        ...
        //封面图片
        QImage picImage = player->metaData(QStringLiteral("CoverArtImage")).value<QImage>(); // 也有可能是ThumbnailImage
        if(picImage.isNull())
            picImage = QImage(":/image/image/image/non-music.png");
        //message
        mySystemTray->setToolTip("正在播放：" + info);
        mySystemTray->showMessage("正在播放...", info, QIcon(QPixmap::fromImage(picImage)), 10000); // 真有图标效果吗？
...
void MainWidget::systemTrayIcon_activated(QSystemTrayIcon::ActivationReason reason)
{
    switch (reason) {
        ...
        case QSystemTrayIcon::DoubleClick:
            QMessageBox::about(this, "0000", "doubleclicked");
            break;
        */
...
/**
 * @brief  init_systemTrayIcon 初始化系统托盘
 */
void MainWidget::init_systemTrayIcon()
{
    ...
    //添加菜单项
    QAction *action_systemTray_show_main = new QAction(QIcon(":/image/image/image/main.png"), u8"显示窗口");
    connect(action_systemTray_show_main, &QAction::triggered, this, &MainWidget::on_btnShowMain_clicked);
    ...
    pContextMenu->addAction(action_systemTray_show_main);
...
/**
 * @brief  mousePressEvent 实现点击界面中某点，音量条隐藏
 *
 * @param  event
 */
...
void MainWidget::on_btnShowMain_clicked()
{
    if (isHidden()) {
        show();
        QDesktopWidget *desktop = QApplication::desktop();
...
/**
 * @brief  on_btnCurMusic_clicked 切换到“当前播放”界面
 */
...
/**
 * @brief  on_btnLocalMusic_clicked 切换到“本地音乐”界面
 */
...
/**
 * @brief  on_btnFavorMusic_clicked 切换到“我喜欢”界面
 */
...
/**
 * @brief  on_btnNetWorkMusic_clicked 切换到“网络歌曲”界面
 */
void MainWidget::on_btnNetWorkMusic_clicked()
{
...
/**
 * @brief  init_entity 注册元类型
 */
void MainWidget::init_entity()
{
    qRegisterMetaType<SongSearchResult>("SongSearchResult");
}

void MainWidget::on_btnAbout_clicked()
{
    QMessageBox::about(this,u8"关于","MusicPlayer | 一款精致小巧的音乐播放器\n"
                                   "【歌词文件说明】需要与对应的歌曲MP3在同目录且同名（.lrc文件）\n"
                                   "【快捷键说明】\n"
                                   "  播放/暂停  -  空格\n"
                                   "  上一曲/下一曲  -  Alt键+方向键←/→\n"
                                   "【添加本地音乐】可直接拖拽至软件界面内或者点击本地音乐界面的添加按钮（Ctrl键+O）\n"
                                   "【音乐文件类型】可播放的文件类型: .mp3/.flac/.mpga文件，使用\"Ctrl+A\"选择文件夹内全部文件添加即可\n"
                                   "【搜索网络歌曲】使用的网易云音乐API，可下载歌曲，下载歌曲的同时会自动下载歌词，不能在线播放，下载前可以查看歌词\n");
}
